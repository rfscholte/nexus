    ...
    extends AbstractNexusIntegrationTest
{
        ...
        throws Exception
    {
        ...
        AssertJUnit.assertTrue( "Expected task to be created: ", TaskScheduleUtil.create( scheduledTask ).isSuccess() );
        
        // now check the conf
        List<CScheduledTask> tasks = NexusConfigUtil.getNexusConfig().getTasks();
        AssertJUnit.assertTrue( "Expected at least 1 task in nexus.xml", tasks.size() > 0 );
        
        for ( CScheduledTask task : tasks )
        {
            AssertJUnit.assertFalse( "Found package name in nexus.xml for task type: "+ task.getType(), task.getType().contains( "org.sonatype." ));
        }
